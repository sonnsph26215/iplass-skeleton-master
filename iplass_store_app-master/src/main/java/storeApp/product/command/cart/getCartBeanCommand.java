...
public class getCartBeanCommand implements Command {
	...
	public String execute(RequestContext request) {
		...
		if (cartBean != null && cartBean.getCartSize() > 0) {
			...
			request.setAttribute("totalprice", cartBean.getTotalPrice());
			System.out.print("------------" + cartBean.getTotalPrice());
		}
		
		String[] properties = new String[] { OrderProduct.OID, OrderProduct.NAME, OrderProduct.ORDER_TOTAL_PRICE};
		List<OrderProduct> orderProducts = EntityDaoHelper.searchDistinctEntity(OrderProduct.DEFINITION_NAME, null, properties);
